...
def calculate_metrics(args):
    ...
    for i in tqdm(range(n)):
        ...
        # 检查图像尺寸并动态调整win_size
        min_dimension = min(img_gt.shape[0], img_gt.shape[1])
        if min_dimension < 7:
            # 如果图像太小，设置win_size为最小维度（必须是奇数）
        ...
        # 使用修正后的参数计算SSIM
        try:
            ssim += compare_ssim(img_gt, img_input, multichannel=True, win_size=win_size, channel_axis=-1)
        except ValueError as e:
            print(f"Warning: SSIM calculation failed for image {i}: {e}")
            print(f"Image shape: {img_gt.shape}, using win_size=3")
            ssim += compare_ssim(img_gt, img_input, multichannel=True, win_size=3, channel_axis=-1)
